...
class REINFORCE(BaseAgent):
    ...
    def __init__(self, config):
        ...
        self.policy = self.create_NN(input_dim=self.get_state_size(), output_dim=self.action_size)
    ...
    def pick_action_and_get_log_probabilities(self):
        ...
        action_probabilities = self.policy.forward().cpu()
    ...
    def calculate_policy_loss_on_episode(self, total_discounted_reward):
        ...
        policy_loss = torch.cat(
            policy_loss).sum()  # We need to add up the losses across the mini-batch to get 1 overall loss
        return policy_loss
