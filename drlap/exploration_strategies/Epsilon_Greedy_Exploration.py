...
class Epsilon_Greedy_Exploration(Base_Exploration_Strategy):
    ...
    def perturb_action_for_exploration_purposes(self, action_info):
        ...
        top_k = action_info["top_k"]
        pass_actions = action_info.get("pass_actions")
        ...
        if (random.random() > epsilon or turn_off_exploration) and (episode_number >= self.random_episodes_to_run):
            ...
            return torch.topk(action_values, top_k).indices
        return np.random.choice(action_values.size(-1), size=(action_values.size(0), top_k), replace=False)
