...
class Memory_Shaper(object):
    ...
    def put_adapted_experiences_in_a_replay_buffer(self, action_id_to_actions):
        ...
        if self.action_balanced_replay_buffer:
            ...
            replay_buffer = Action_Balanced_Replay_Buffer(self.buffer_size, self.batch_size, self.seed,
        ...
        for episode_ix in range(episodes):
            self.add_adapted_experience_for_an_episode(episode_ix, actions_to_action_id, max_action_length,
                                                       replay_buffer)
    ...
    def add_adapted_experience_for_an_episode(self, episode_ix, action_rules, max_action_length, replay_buffer):
        ...
        assert len(states) == len(next_states) == len(rewards) == len(dones) == len(
            actions), "{} {} {} {} {} = {}".format(len(states), len(next_states), len(rewards), len(dones),
                                                   len(actions), actions)
        steps = len(states)
        for step in range(steps):
            ...
            for action_length in range(2, max_action_length + 1):
                ...
                assert all([action in range(self.num_actions) for action in
                            action_sequence]), "All actions should be primitive here"
